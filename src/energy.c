...
static double energy_unshared( const shell *s, const shell_params *sp,
  ...
  double *x0, *x1;
  double b, db_dx0[3], db_dx1[3], db_dx2[3], db_dx3[3];
  double k_s, k_b, th0;
  ...
  h = k_b*(b-1)*(b-1);
  for( i=0; i<3; i++){
    dx[3*vi0+i] += 2.*k_b*(b-1)*db_dx0[i]
    dx[3*vi1+i] += 2.*k_b*(b-1)*db_dx1[i]
  }
...
static double energy_shared( const shell *s, const shell_params *sp,
  ...
  vi0 = s->l[i].i[0];
  vi1 = s->l[i].i[1];
  vi2 = other_index( s, s->ld[i].t[0], vi0, vi1);
  vi3 = other_index( s, s->ld[i].t[1], vi0, vi1);
  ...
  dihedral_d( x1, x2, x3, x4, &co, &si,
             dco_dx0, dco_dx1, dco_dx2, dco_dx3,
             dsi_dx0, dsi_dx1, dsi_dx2, dsi_dx3);
  h += k_s*(1-co*cos(th0)-si*sin(th0));
  for( i=0; i<3; i++){
    dx[3*vi0+i] += k_s*(-cos(th0)*dco_dx0[i] - sin(th0)*dsi_dx0[i]);
    dx[3*vi1+i] += k_s*(-cos(th0)*dco_dx1[i] - sin(th0)*dsi_dx1[i]);
    dx[3*vi2+i] += k_s*(-cos(th0)*dco_dx2[i] - sin(th0)*dsi_dx2[i]);
    dx[3*vi3+i] += k_s*(-cos(th0)*dco_dx3[i] - sin(th0)*dsi_dx3[i]);
  }
